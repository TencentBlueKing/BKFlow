# BKFlow dev_multi_tenant 源码审查知识

整理基线：`8d09251ba37ec4303d9528e2fb2c578dffb392ad`，目标分支：`dev_multi_tenant`。此文件描述该基线的真实路径与行为；审查时应结合当前 PR base/head 重新核对，不能将其他分支后来新增的能力直接当成本分支契约。

依赖基线来自 `requirements.txt`：`Django==3.2.25`；`celery==5.2.3`；`blueapps==4.16.rc3`；`bamboo-pipeline==3.29.7`。前端 Vue/画布 SDK 版本与锁文件以 `frontend/package.json` 为准。本分支未提供 test:plugin-form 专项脚本，不应套用其他分支的命令。

## dev_multi_tenant 的实际功能边界

多租户已是本分支源码中的现有行为。`bkflow/space/models.py` 的 Space 有 tenant_id 和 tenant_mode，`bkflow/task/models.py` 的 TaskInstance 有 tenant_id；对应迁移是 `bkflow/space/migrations/0012_auto_20260512_1102.py` 与 `bkflow/task/migrations/0016_taskinstance_tenant_id.py`。旧数据默认租户为 default，不能仅凭默认值断言越权，也不能把历史缺字段问题当成本次新增缺陷。

`bkflow/space/views.py` 的空间查询存在基于 request.user.tenant_id 的过滤。`bkflow/template/views/template.py` 的任务创建路径传入当前用户租户，`bkflow/apigw/views/create_task.py` 与 `bkflow/apigw/views/create_task_by_app.py` 从空间取租户；`bkflow/task/serializers.py` 声明任务创建的租户字段。应逐入口检查身份来源、空间授权、任务持久化和子任务继承，不能把客户端任意传入的 tenant_id 当成可信用户身份。

`bkflow/utils/context.py` 从任务建立租户上下文；`bkflow/pipeline_plugins/components/collections/remote_plugin/v1_0_0.py` 向插件客户端传入 tenant_id；`bkflow/pipeline_plugins/components/collections/approve/v1_0.py`、`bkflow/interface/itsm/itsm.py`、`bkflow/utils/message_cmsi.py` 有向外部系统传递 X-Bk-Tenant-Id 的路径。审查后台任务、回调、通知和第三方调用时，应继续跟踪这些路径，不能仅检查入口参数。

`module_settings.py` 加载租户初始化应用，`bkflow/contrib/init_tenant/management/commands/init_tenant.py` 调用 ITSM 工作流迁移命令；这类命令会产生外部写入，不能用作自动 review 的试探性验证。当前未包含子画布插件或 Uniform API V4；blueapps 依赖为 4.16.rc3，多租户验收需要匹配身份系统、网关和下游系统的实际版本。
