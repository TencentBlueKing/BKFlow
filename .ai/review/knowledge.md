# BKFlow feat/replace_canvas_sdk 源码审查知识

整理基线：`10b9b0b9323c23bd0bd8990a41a5612656c8ff19`，目标分支：`feat/replace_canvas_sdk`。此文件描述该基线的真实路径与行为；审查时应结合当前 PR base/head 重新核对，不能将其他分支后来新增的能力直接当成本分支契约。
...
## feat/replace_canvas_sdk 的实际功能边界

`frontend/package.json` 将 `@blueking/bkflow-canvas-editor-vue2` 固定为 `1.2.0-beta.1`。`frontend/src/views/template/TemplateEdit/index.vue` 接入 FlowEditBridge、FlowViewBridge、FlowDebugBridge、FlowCreateTaskBridge；`frontend/src/views/task/TaskExecute/TaskOperation.vue` 接入 FlowExecuteBridge。审查 Bridge 参数、事件、生命周期清理、编辑/只读/调试/执行模式时，应比较本分支实际组件用法和锁文件，不能套用旧 StageCanvas 的事件约定。

`frontend/src/views/template/TemplateEdit/flowEditApi.js` 承接模板保存、插件元数据、任务创建和节点操作的协议适配。保存通过方法覆盖头传递 PATCH，节点操作把 forceFail/resume/gatewaySkip 映射为 forced_fail/callback/skip_exg。修改映射应核对后端路由、序列化字段和返回数据，尤其是 space_id、任务/节点 ID、token 的继承及嵌套子树。`frontend/src/utils/validatePipeline.js` 是前端校验入口之一；前端校验通过仍不能替代保存端与任务端的校验。

后端包含 `bkflow/pipeline_plugins/components/collections/subcanvas_plugin/v1_0_0.py` 和 `bkflow/pipeline_plugins/components/collections/uniform_api/v4_0_0.py`，旧组件版本仍有独立契约。Bridge 迁移需要覆盖已有模板加载、编辑后保存再读取、调试、创建任务和运行时操作；仅构建成功不足以证明这些交互可用。
