# BKFlow feat/a2flow 源码审查知识

整理基线：`8a648f09f6192074eb4f71bb261edb864d6ce1f9`，目标分支：`feat/a2flow`。此文件描述该基线的真实路径与行为；审查时应结合当前 PR base/head 重新核对，不能将其他分支后来新增的能力直接当成本分支契约。
...
本分支围绕 A2Flow 转换与流程树校验演进。`bkflow/pipeline_validate/handler.py` 的 ValidatorHandler 注册校验器，未指定 ValidateType 时执行全部校验器，指定类型时选择该类型加 GENERAL，并在无效结果时抛出 PipelineException。`bkflow/pipeline_validate/validators/template.py` 包含 schema、变量 key/source_info、输出 key、节点策略互斥等校验；任务和通用校验分别位于 `bkflow/pipeline_validate/validators/task.py`、`bkflow/pipeline_validate/validators/general.py`。新增校验必须追踪注册、实际调用位置和传入的树，不能只因类存在就认定所有入口已覆盖。

需要沿 A2Flow 输入解析、转换后的树、模板快照/发布及任务创建确认有效数据；特别关注旧数组协议与 v2 对象协议、插件输入/输出映射、网关与循环变量作用域、用户可见的结构化错误格式。当前仓库没有 Harness 业务模块，不应从历史设计或其他工作区导入 TokenLease、ExecutionSaga、外部 provider 等实现假设。

本分支包含 `bkflow/pipeline_plugins/components/collections/uniform_api/v4_0_0.py`，未包含子画布插件实现。转换器名称和同名源码存在不代表与最新主分支行为一致。
