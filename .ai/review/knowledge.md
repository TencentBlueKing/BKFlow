# BKFlow feat/space-config-redesign 源码审查知识

整理基线：`dea2ac2cad5553adb6733fca7f4a2598e511a242`，目标分支：`feat/space-config-redesign`。此文件描述该基线的真实路径与行为；审查时应结合当前 PR base/head 重新核对，不能将其他分支后来新增的能力直接当成本分支契约。

依赖基线来自 `requirements.txt`：`Django==3.2.25`；`celery==5.2.3`；`blueapps==4.15.8`；`bamboo-pipeline==3.29.9`。前端 Vue/画布 SDK 版本与锁文件以 `frontend/package.json` 为准。本分支另外提供 `npm run test:plugin-form`，具体覆盖范围按 package.json 中实际脚本判断。

## feat/space-config-redesign 的实际功能边界

本分支的配置入口为 `bkflow/space/configs.py`、`bkflow/space/serializers.py`、`bkflow/space/views.py`，编辑 UI 为 `frontend/src/views/admin/Space/SpaceConfig/ConfigDetail.vue`。该 UI 支持结构化表单与 JSON 源码、默认值/自定义值及恢复默认。审查需要跟踪表单值转换、服务端校验、持久化与再次读取的完整链路，避免只验证控件显示。

`bkflow/plugin/space_plugin_config_parser.py` 的 SpacePluginConfigParser 要求 default 配置，并按 scope 覆盖或回退 default；allow_list 取交集、deny_list 排除、allow_all 保留全部。插件集合与 QuerySet 有两条过滤路径，改变规则时需确认两者一致。`bkflow/pipeline_plugins/query/uniform_api/uniform_api.py` 和 `bkflow/pipeline_plugins/components/collections/uniform_api/v4_0_0.py` 负责相关查询/执行行为，配置可见性不自动等于执行权限。

当前包含 Uniform API V4，未包含子画布插件实现；不要将其他分支的子画布运行链作为本分支的现有能力或强制修复目标。
