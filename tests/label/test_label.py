...
class TestLabelSerializer:
    """Tests for label serializer validation logic."""
    ...
    def test_validate_color_valid_and_invalid_formats(self):
        ...
        from bkflow.label.serializers import LabelCreateSerializer

        factory = APIRequestFactory()
        request = factory.post("/api/label/", data={"space_id": 1})

        # Test individual validation method
        serializer = LabelCreateSerializer(context={"request": request})
    ...
    def test_validate_name_duplicate_check(self):
        ...
        from bkflow.label.serializers import (
    ...
    def test_validate_label_scope_update_parent_must_cover_children(self):
        """Updating a parent label should require scope to cover all descendant scopes."""
        from rest_framework.test import APIRequestFactory

        from bkflow.label.serializers import LabelUpdateSerializer

        parent = make_label("parent_upd", label_scope=["task", "template"], color="#ffffff", space_id=1)
        child1 = make_label("child1_upd", parent_id=parent.id, label_scope=["task"], color="#ffffff", space_id=1)
        make_label("child2_upd", parent_id=parent.id, label_scope=["common"], color="#ffffff", space_id=1)
        make_label("grandchild_upd", parent_id=child1.id, label_scope=["template"], color="#ffffff", space_id=1)

        factory = APIRequestFactory()
        request = factory.put(f"/api/label/{parent.id}/", data={"space_id": 1})

        # invalid: does not cover children's union scope (missing 'common')
        serializer = LabelUpdateSerializer(
            instance=parent,
            data={"space_id": 1, "label_scope": ["task", "template"], "color": "#ffffff"},
            context={"request": request},
            partial=True,
        )
        with pytest.raises(DRFValidationError) as exc:
            serializer.is_valid(raise_exception=True)
        assert "父标签的范围必须覆盖所有子标签的范围" == exc.value.detail["label_scope"][0]

        # valid: covers all children scopes
        serializer = LabelUpdateSerializer(
            instance=parent,
            data={"space_id": 1, "label_scope": ["task", "template", "common"], "color": "#ffffff"},
            context={"request": request},
            partial=True,
        )
        assert serializer.is_valid() is True

    def test_create_child_label_forbidden_when_parent_is_referenced(self):
        """Creating a child label should fail if selected parent label is referenced by template/task."""
        from rest_framework.test import APIRequestFactory

        from bkflow.label.serializers import LabelCreateSerializer

        parent = make_label("parent_ref", label_scope=["task"], color="#ffffff", space_id=1)
        # simulate template reference on parent
        TemplateLabelRelation.objects.create(template_id=100, label_id=parent.id)
    ...
    def test_create_child_label_forbidden_when_parent_is_task_referenced_via_client(self, monkeypatch):
        ...
        from bkflow.label import serializers as label_serializers
        from bkflow.label.serializers import LabelCreateSerializer

        parent = make_label("parent_task_ref", label_scope=["task"], color="#ffffff", space_id=1)
        ...
        class FakeClient:
            ...
            def get_task_label_ref_count(self, space_id, label_ids):
                return {"result": True, "data": {str(parent.id): 1}}

        monkeypatch.setattr(label_serializers, "TaskComponentClient", FakeClient)
    ...
    def test_create_child_label_forbidden_when_parent_is_a_child(self):
        ...
        root = make_label("root", label_scope=["task"], color="#ffffff", space_id=1)
        child = make_label("child", parent_id=root.id, label_scope=["task"], color="#ffffff", space_id=1)

        factory = APIRequestFactory()
        request = factory.post("/api/label/", data={"parent_id": child.id})
        ...
        assert "子标签不能作为父标签" == exc.value.detail["parent_id"][0]
