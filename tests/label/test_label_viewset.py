...
class TestLabelViewSet:
    ...
    def test_create_child_forbidden_when_parent_descendant_is_referenced(self):
        ...
        payload = {
            "name": "parent_ref_guard/new_child",
            "space_id": 1,
            "color": "#123456",
            "label_scope": ["task"],
        }
        request = self.factory.post("/api/label/", payload, format="json")
        request.user = self.admin_user

        with patch("bkflow.label.serializers.TaskComponentClient") as mock_client:
            mock_instance = mock_client.return_value
            mock_instance.get_task_label_ref_count.return_value = {"result": True, "data": {}}
