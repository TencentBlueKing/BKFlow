...
class TestOperateTask(TestCase):
    ...
    def test_start_task_by_app_rejects_ungranted_open_plugin(self, mock_client_class):
        """按应用启动入口必须在调用 engine 前重新校验来源准入。"""
        pipeline_tree = build_open_plugin_pipeline_tree()
        create_open_plugin_catalog(space_id=self.space.id, enabled=True)
        ...
        with pytest.raises(serializers.ValidationError, match="来源"):
            unwrap(operate_task_by_app)(request, task_id=1, operation="start")
