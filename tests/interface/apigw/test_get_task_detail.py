...
class TestGetTaskDetail(TestCase):
    ...
    def test_get_task_detail_success(self, mock_client_class, mock_get_history):
        """测试正常获取任务详情并附带 webhook 投递历史"""
        space = self.create_space()
        mock_client = mock_client_class.return_value
        mock_client.get_task_detail.return_value = {
            "result": True,
            "data": {"id": 123, "name": "test_task"},
        }
    ...
    @mock.patch("bkflow.apigw.views.get_task_detail.get_webhook_delivery_history_by_delivery_id")
    @mock.patch("bkflow.apigw.views.get_task_detail.TaskComponentClient")
    def test_get_task_detail_with_empty_webhook_history(self, mock_client_class, mock_get_history):
        """测试 webhook 投递历史为空列表的情况"""
        space = self.create_space()
        mock_client = mock_client_class.return_value
        mock_client.get_task_detail.return_value = {
            "result": True,
            "data": {"id": 456, "name": "no_webhook_task"},
        }
        mock_get_history.return_value = []
    ...
    @mock.patch("bkflow.apigw.views.get_task_detail.get_webhook_delivery_history_by_delivery_id")
    @mock.patch("bkflow.apigw.views.get_task_detail.TaskComponentClient")
    def test_get_task_detail_task_id_converted_to_str(self, mock_client_class, mock_get_history):
