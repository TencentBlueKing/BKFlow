...
class TestTemplateViewsMoreCoverage:
    ...
    def test_update_labels_action_invalid_payload(self):
        """Invalid label_ids should be rejected before updating template labels."""
        view = TemplateViewSet.as_view({"post": "update_labels"})
        request = self.factory.post(
            f"/templates/{self.template.id}/update_labels/", {"label_ids": ["bad"]}, format="json"
        )
        force_authenticate(request, user=self.user)
        response = view(request, pk=self.template.id)
        ...
        assert response.status_code == 200
        assert response.data["result"] is False
        assert "label_ids" in response.data["data"]
        assert not TemplateLabelRelation.objects.filter(template_id=self.template.id).exists()
