...
class TestOpenPluginV4Demo:
    ...
    def test_list_meta_returns_v4_catalog_fields(self):
        ...
        request = self.factory.get("/api/api_plugin_demo/open_plugin_v4/list_meta/")
        response = views.open_plugin_v4_list_meta_api(request)
        ...
        assert polling_plugin["meta_url_template"].endswith(
            "/api_plugin_demo/open_plugin_v4/detail_meta/?api_id=demo_polling_job&version={version}"
        )

    def test_detail_meta_returns_selected_version_and_schedule_config(self):
        """测试 v4 示例详情支持按业务版本返回 schema 和调度配置。"""
        request = self.factory.get(
            "/api/api_plugin_demo/open_plugin_v4/detail_meta/",
            {"api_id": "demo_callback_job", "version": "2.0.0"},
        )
        response = views.open_plugin_v4_detail_meta_api(request)
        ...
        assert detail["url"].endswith("/api_plugin_demo/open_plugin_v4/execute/")
    ...
    def test_execute_accepts_context_and_returns_run_id(self):
        ...
        request = self.factory.post(
            "/api/api_plugin_demo/open_plugin_v4/execute/",
            {
                "source_key": "demo",
        ...
        response = views.open_plugin_v4_execute_api(request)
    ...
    def test_status_returns_outputs_for_polling_validation(self):
        ...
        request = self.factory.get(
            "/api/api_plugin_demo/open_plugin_v4/status/",
            {"task_tag": "demo_polling_job:task-1-node-node-a-attempt-1", "status": "SUCCEEDED"},
        )
        response = views.open_plugin_v4_status_api(request)
