...
class TaskOperation:
    ...
    def start(self, operator: str, *args, **kwargs) -> OperationResult:
        ...
        try:
            ...
            # 这样插件执行时可以复用 start_task 的 trace_id
            trace_context = get_current_trace_context()
            if trace_context:
                root_pipeline_data["_trace_id"] = trace_context["trace_id"]
                root_pipeline_data["_parent_span_id"] = trace_context["span_id"]
                logger.info(
                    f"[TaskOperation.start] Captured trace context for task {self.task_instance.id}: "
                    f"trace_id={trace_context['trace_id']}, parent_span_id={trace_context['span_id']}"
                )
