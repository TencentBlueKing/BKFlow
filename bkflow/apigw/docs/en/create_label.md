...
| --- | --- | --- | --- |
| name | string | Yes | Tag name. Supports `parent/child`: supplying `a/b` creates or reuses the root tag `a` and creates child tag `b` under it. |
| color | string | Yes | Tag color, such as `#ffffff` |
| label_scope | list | Yes | Tag scopes (multiple allowed): `task`, `template` |
...
| full_path | string | Full tag path, such as `parent/child` |
