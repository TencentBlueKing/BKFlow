...
- **申请新 token**：当指定的资源（resource_type + resource_id + permission_type）不存在有效 token 时，会创建一个新的 token
- **返回已有 token**：当指定资源已存在未过期的 token 时，会返回该 token
- **自动续期**：如果空间配置中开启了 `token_auto_renewal`（token 自动续期），每次获取已有 token 时会自动刷新其过期时间
- **过期时间**：token 的有效期由空间配置 `token_expiration` 决定（如 1h、24h 等）
...
可通过 `renew_space_config` 接口配置上述选项。