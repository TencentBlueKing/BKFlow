...
多租户模式开启时必须传入已核实归属的 `tenant_id`；关闭时旧请求可省略或传入空字符串，使用 `default`。
...
| tenant_id | string | 条件必填 | 租户 ID，最多 32 位；多租户开启时必填且必须等于应用本次请求租户 |
| tenant_mode | string | 否 | 固定为 `single`，默认 `single`；当前不支持全租户空间 |
