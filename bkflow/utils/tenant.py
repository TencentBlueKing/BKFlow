...
def get_request_tenant_id(request):
    ...
    if not isinstance(tenant_id, str) or not tenant_id.strip():
        raise PermissionDenied(_("当前用户缺少租户身份"))
...
def get_apigw_request_tenant_id(request):
    ...
    if not app or not app.verified:
        raise PermissionDenied(_("缺少已认证的网关应用身份"))
...
def get_task_tenant_id(parent_data):
    ...
    if not tenant_id:
        raise PermissionDenied(_("任务缺少租户归属，请先完成存量数据回填"))
    return tenant_id
            ...
            if space_id != "0":
                ...
                for data in sources:
                    if data.get("space_id") is not None and str(data["space_id"]) != str(space_id):
                        raise PermissionDenied(_("请求空间与内部调用空间不一致"))
