...
class LabelViewSet(AdminModelViewSet):
    ...
    def get_serializer_class(self):
        ...
        if self.action in ["update", "partial_update"]:
            from .serializers import LabelUpdateSerializer
    ...
    def get_label_ref_count(self, request, *args, **kwargs):
        ...
        # 过滤所有子标签
        all_child_label_ids = Label.objects.filter(
            space_id=validated_params["space_id"], parent_id__in=label_ids
        ).values("id", "parent_id")
        label_child_map = {}
        ...
        all_query_label_ids = list(
            set(label_ids + [str(num) for value_list in label_child_map.values() for num in value_list])
        )
        # 调用任务组件接口获取任务引用数量
        ...
        for label_id in label_ids:
            if label_id in label_child_map:
                # 如果存在子标签，统计子标签的引用数量
                child_label_ids = label_child_map[label_id]
                template_count = sum(label_template_count_map.get(int(child_id), 0) for child_id in child_label_ids)
                task_count = sum(label_task_ref_count_map.get(str(child_id), 0) for child_id in child_label_ids)
            else:
