...
class HttpRequestService(BKFlowBaseService):
    ...
    def plugin_schedule(self, data, parent_data, callback_data=None):
        ...
        try:
            ...
            err = _("请求异常，详细信息: {msg}").format(msg=e)
        ...
        if not (200 <= response.status_code < 300):
            err = _("请求失败，状态码: {status_code}，响应: {resp}").format(status_code=response.status_code, resp=resp)
        ...
        if success_exp:
            try:
                ...
                err = _("请求成功条件判定出错: {msg}").format(msg=e)
                self.logger.error(err.format(traceback.format_exc()))
                data.outputs.ex_data = err.format(e)
                return False
...
class HttpComponent(Component):
    ...
    version = "v1.0"
    form = "%scomponents/http/v1_0.js" % settings.STATIC_URL
