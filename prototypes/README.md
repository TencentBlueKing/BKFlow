...
设计新原型或大幅调整页面时，应优先参考蓝鲸标准设计规范。若当前 AI 会话能访问 `bkui-knowledge`，可读取相关页面模式、组件 API 和交互示例作为设计输入；若无法访问，则以本仓库的 `examples/component-showcase.html`、`masters/` 母版和真实前端相近页面为基线。

这里的参考只用于决定页面结构、组件选择、交互模式和状态表达。最终产物仍然落在 `features/` 或 `masters/`，使用本目录 `assets/` 下的静态 CSS/JS；不要恢复旧的 `output/`、`metadata/`、`cache/` 生成产物目录，也不要引入 Vue/React、bkui-vue CDN 或 npm 构建链路。
