...
> **Related PR:** [#593](https://github.com/TencentBlueKing/BKFlow/pull/593)

**Goal:** 为节点（插件）执行建立完整的 OpenTelemetry Span 构建与链路追踪能力，支持跨 schedule 调用的 trace 连续性。

**Architecture:** 在任务启动时捕获外部 trace context 并注入 pipeline data，节点基类统一构建 span，通过 data.outputs 持久化 span 信息实现跨 schedule 追踪，使用 NonRecordingSpan 重建 parent context。

**Tech Stack:** OpenTelemetry, Django, bamboo_engine (Pipeline), Celery
