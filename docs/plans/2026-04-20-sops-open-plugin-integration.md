...
├── bkflow/apigw/docs/zh/operate_task.md
...
├── tests/fixtures/uniform_api_v4/
│   └── list_meta.json / detail_meta.json / execute.json / callback.json 协议基线
└── tests/
...
│   ├── views.py
│   ├── tasks.py
│   ├── management/commands/sync_open_plugin_catalog.py
│   ├── fixtures/uniform_api_v4/
│   │   └── list_meta.json / detail_meta.json / execute.json / callback.json 协议基线
│   └── services/
│       ├── catalog.py
│       ├── execution.py
...
def test_validate_v4_detail_meta_polling_contract():
    client = UniformAPIClient()
    data = {
        "id": "open_plugin_001",
        "name": "JOB 执行作业",
...
- [ ] **Step 3.5: 固化 v4 协议 fixture**

在 `tests/fixtures/uniform_api_v4/` 下新增：
...
BKFlow 与标准运维两侧测试都以同名 fixture 作为协议基线，避免字段拼写漂移。
...
- Modify: `../bk-sops/gcloud/open_plugins/serializers.py`
- Modify: `../bk-sops/gcloud/open_plugins/urls.py`
- Create: `../bk-sops/gcloud/open_plugins/tasks.py`
- Create: `../bk-sops/gcloud/open_plugins/management/commands/sync_open_plugin_catalog.py`
...
- `POST /open-plugin-runs/{run_id}/cancel` 取消与幂等取消
...
- [ ] **Step 3: 落表 `OpenPluginSourceConfig` 与 `OpenPluginRun`**

在 `models.py` 中定义最小字段：

```python
class OpenPluginSourceConfig(models.Model):
    ...
    phase1_plugin_allow_list = models.JSONField(default=list)
...
同时新增迁移文件。`default_project_id`、callback 域白名单与一期开放白名单的权威源在标准运维侧，只在这里存一份，BKFlow 不重复保存。
...
- 对 `OpenPluginSourceConfig` 应用 `default_project_id` 与一期开放白名单
...
- [ ] **Step 6: 实现状态查询、取消接口与回调桥接**

在 `views.py` / `services/callbacks.py` 中补齐：

- `POST /open-plugin-runs`
- `GET /open-plugin-runs/status?task_tag=...`
- `GET /open-plugin-runs/{run_id}`
- `POST /open-plugin-runs/{run_id}/cancel`
- 对 `callback_url` 做域白名单校验
- 对 `run_id` 查询做接入方归属校验
- 节点已终态时回调按幂等方式吞掉，不产生副作用
- 对大输出体做统一截断并追加 `truncated=true` / `truncated_fields` 标记
...
- “一键全开”只作用于当前已发现插件
- 后续目录新增插件仍默认关闭
- 远端下线插件后目录索引标记 `unavailable`，但历史开放记录保留
- `plugin_id` 变化按新插件处理
...
不要新建第二套来源配置体系。callback 域白名单与 phase-1 开放白名单由标准运维侧 `OpenPluginSourceConfig` 维护，BKFlow 不做权威配置副本。
...
- 远端下线插件时在目录索引中标记 `unavailable`，不删 `SpaceOpenPluginAvailability`
- 远端新增插件时为所有关联空间补 `enabled=false`
- `plugin_id` 改变按新插件处理

在 `bkflow/plugin/tasks.py` 中新增周期任务 `sync_open_plugin_catalog`，并提供按空间/按来源触发的同步入口。
...
- 切换单个插件 `enabled`
- 对当前可见插件执行“一键全开”
- 按来源批量置 `enabled=false` 作为 kill switch
...
- Modify: `bkflow/apigw/docs/zh/operate_task.md`
...
- 服务端在模板保存 / 任务创建 / 启动任务时校验当前空间是否仍开放该插件
- 模板保存与任务创建时写入 `plugin_reference_snapshot + plugin_schema_snapshot`
...
- [ ] **Step 4: 在模板保存、任务创建和启动入口增加治理校验并写入快照**

在 `bkflow/template/serializers/template.py`、`bkflow/apigw/views/create_task.py` 与 `bkflow/apigw/views/operate_task.py` 中复用同一校验函数：

- 旧模板如果引用已关闭插件，可查看但不能继续新建任务
- 新建任务必须命中当前空间开放状态
- 任务启动前再次校验运行期仍可执行
- 模板保存时写入 `plugin_reference_snapshot`
- 任务创建时复制并固化 `plugin_reference_snapshot + plugin_schema_snapshot`
...
- 更新 `bkflow/apigw/docs/zh/operate_task.md`
...
- 收到 `WAITING_CALLBACK` 状态时停止继续轮询，转入等待回调
...
- 已有快照记录不会被 backfill 覆盖
- 历史节点缺少 `wrapper_version` 时可按来源声明补齐
- 失效版本仍可回看，不可继续新建任务
...
- [ ] **Step 3: 实现历史快照读取与只读判定**

在 `open_plugin_snapshot.py` 中封装：
...
- 为历史模板和任务补缺失的 `plugin_schema_snapshot`
- 根据来源当前声明的 `wrapper_version` 回填历史节点缺失的 `wrapper_version`
- 已有非空快照一律跳过，不覆盖
- 不自动升级到最新版本
- `--dry-run` 先输出差异报告
...
映射关系：**plan P0 ⊇ spec P0 + spec P1 的最小可用集；plan P1 ⊇ spec P2 的后向兼容与运营补齐。**
...
- `client_request_id` 幂等成立，BKFlow 重试不会在标准运维侧生成重复执行
- `open_plugin_run_id ↔ bkflow_node_id` 映射可追踪并可用于排障
- `WAITING_CALLBACK` 状态下 BKFlow 不继续轮询
- 大输出体被截断时带有显式截断标记，不静默丢数据
