...
接入系统应先在后端检查用户的业务权限，再以该用户身份申请资源票据。BKFlow 页面仍需要登录态，票据用于约束资源访问。资源与权限组合、页面传递、续期撤销及当前实现边界见 [Token 授权与接入说明](token_authorization.md)。

接入方需要让一个角色同时获得模板调试、任务操作等能力时，可按 [组合 Token 接入改造指南](composite_token_integration.md) 修改角色映射、申请封装、缓存、页面传递和撤销逻辑。

旧单项 HTTP 请求和已签发 token 值在统一存储版本继续兼容，但旧服务进程不能与删除旧字段后的数据库混跑。升级需在维护窗口停止旧实例并完成自动回填，确认旧票据读回后再恢复服务；详见 [Token 上线与回退](token_authorization.md#11-上线与回退)。
