...
本文的授权行为以 2026-09-08 核对的主干提交 `032d59aafcb36d45af4f18d52d89c082b2d2fd27` 为基线；代码类型名称采用下文说明的拆分后定义，拆分不改变票据值或接口协议。部署版本不同时，应核对对应版本的权限类和配置；本文不表示生产部署或安全验收已完成。
...
| `user` | 被授权用户名 |
| `resource_type` | 资源类型 |
| `resource_id` | 资源标识，字符串，最长 32 位 |
| `permission_type` | 单个权限类型 |
| `expired_time` | 服务端过期时间 |

例如，同一用户对模板 `100` 的 `VIEW` 与 `EDIT` 是两组不同授权。票据没有绑定浏览器会话、模板版本、流程内容指纹、审批单或调用次数，也没有单独的累计最长寿命字段。
...
| `Token.PERMISSION_TYPE` | `VIEW`、`EDIT`、`OPERATE`、`MOCK` | 由操作类型定义的模型字段 choices，也是申请、撤销接口的权限参数候选值 |
...
旧版本的 `PermissionType` 将上述四种操作和三个 `FLOW_*` 标记放在同一个枚举中。拆分只调整 Python 类型和引用：数据库中的 `permission_type` 值、接口接受的四种权限字符串及前端消费的 `auth` 数组均保持兼容。
...
1. 校验字段及目标资源存在性。
2. 查找同一空间、用户、资源类型、资源 ID 和权限类型下的未过期票据。
3. 若已有票据，选择最晚过期的一张；开启自动续期时，将过期时间更新为“当前时间 + 空间配置时长”。
4. 若没有有效票据，生成新的票据值并保存。

因此，同一用户反复打开同一资源可能复用同一张票据。字段组合没有数据库唯一约束，不能把复用行为理解成并发签发时严格只会产生一条记录。

当前 `Token.to_json()` 返回 `space_id`、`user`、`resource_type`、`resource_id`、`token` 和 `expired_time`，不返回 `permission_type`；接入方应保留申请时使用的权限类型。
...
校验时根据资源的当前 scope 属性判断，而不是使用签发时的资源列表快照。资源迁入或迁出作用域会影响授权范围。当前 `Token.verify()` 的 scope 分支先按资源 ID 查模板，模板不存在时再查任务；它没有额外接收“目标是模板还是任务”的参数，不能将这条查找路径视为所有资源类型都已严格区分的保证。
...
修改 `token_expiration` 不会批量更新已签发票据，只会影响之后的签发或成功续期。30 天上限约束的是配置时长，当前没有独立的累计最长寿命限制；持续成功续期时，总存活时间可以超过单次有效期。续期对过期和撤销状态的实现限制见第 9 节。

撤销入口为 `POST /space/{space_id}/revoke_token/`，支持 `token`、`user`、`resource_type`、`resource_id` 和 `permission_type` 条件，多个条件共同过滤。**提交空对象 `{}` 会撤销该空间全部票据。** 该入口要求应用和网关资源认证，并校验应用与空间绑定，但不要求网关用户认证。
...
撤销与过期清理不会终止已经运行的任务。它们作用于之后的资源访问校验；终止任务需要执行相应任务操作。票据也不与接入系统登录会话绑定，用户退出接入系统不会由上述机制自动触发撤销。
...
模板详情中的 `auth` 汇总当前用户对模板及 scope 持有的全部有效票据权限。任务详情还汇总模板权限，并添加 `FLOW_` 前缀，如 `FLOW_VIEW`、`FLOW_EDIT`、`FLOW_MOCK`。这些是区分模板权限的展示标记，不是 `apply_token` 可签发的新权限类型。
...
以下是本文基线代码中的限制，不应解释为已修复或已在线上验证：

- **续期与撤销状态未完全隔离。** 续期视图的 queryset 在类定义时计算时间，模型的 `renewal()` 本身没有再次检查是否过期，也没有独立撤销标记。因此，已过期或撤销的记录存在被续活的风险；不能仅凭撤销请求成功，就断言原票据永远无法恢复。参见[续期视图](../../bkflow/permission/views.py)和[模型续期方法](../../bkflow/permission/models.py)。
...
- **辅助接口未统一执行用户绑定。** 主模板、任务路径通过 `Token.verify()` 检查用户名；[插件空间权限](../../bkflow/plugin/permissions.py)和[决策表权限](../../bkflow/decision_table/permissions.py)使用单独查询，没有同样的用户名条件。不能概括为所有带 token 的接口都会检查全部绑定维度。
