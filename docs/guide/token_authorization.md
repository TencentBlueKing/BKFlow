...
历史单项票据保留上述资源字段，组合票据通过 `TokenGrant` 保存两条或以上授权明细；用户、空间、过期时间仍只有一份。内部 `grant_set_hash` 用于区分存储形态和复用候选，不对外返回。组合票据明细缺失或摘要不一致时整张无效，不回退读取旧资源字段。
...
新格式响应 data 包含 `space_id`、`user`、`token`、`expired_time`、`grants`，不返回顶层资源字段和内部摘要；旧响应保持原字段集合。去重后仅一项时可复用或创建旧单项存储，仍返回 grants 结构。时间编码沿用申请响应链路，UTC 示例为 `2026-09-09T05:00:00Z`，完整请求/响应见[申请接口说明](../../bkflow/apigw/docs/zh/apply_token.md)。
...
校验时根据资源的当前 scope 属性判断，而不是使用签发时的资源列表快照。资源迁入或迁出作用域会影响授权范围。HTTP 消费者通过 `Token.verify()` 的 `target_resource_type` 明确传入 `TEMPLATE` 或 `TASK`，scope 按实际目标类型查找，避免模板与任务的相同 ID 混淆。只有历史单项票据在未传目标类型的旧式内部调用中保留模板优先、再查任务的回退；组合票据未传目标类型时拒绝该 scope 授权分支。
...
模板详情中的 `auth` 汇总当前用户对模板及 scope 持有的全部有效、授权完整的旧单项和组合票据权限，重复操作只显示一次。任务详情还汇总模板权限，并添加 `FLOW_` 前缀，如 `FLOW_VIEW`、`FLOW_EDIT`、`FLOW_MOCK`。这些是区分模板权限的展示标记，不是 `apply_token` 可签发的新权限类型。
...
1. 先备份数据库并应用增量迁移 `permission.0005`，保留历史资源字段，不批量回填或重签旧票据。升级后的统一读取层可直接读取原单项记录。
2. 部署全部签发、续期、撤销及消费者适配，先保持 `BKAPP_TOKEN_COMPOSITE_ENABLED=false`。确认旧协议、空间/用户隔离、拒绝过期票据及 SDK/页面调用正常。
3. 在对应环境验证完成后，设置 `BKAPP_TOKEN_COMPOSITE_ENABLED=true` 启用新格式；网关沿用现有两条路由、operationId 和认证配置，只更新文档及 Schema，不开启额外请求校验插件。
4. 需暂停新能力时先关闭申请开关。已有组合票据继续有效，关闭开关不是撤销；可按 token、user 或资源条件撤销整票据。
5. 回退到不支持组合票据的旧代码前，先关闭入口，撤销或等待全部组合票据到期并确认不再被消费。保留新增表和可空列，避免直接逆向迁移删除授权数据；旧代码不应消费组合票据。回退到含旧续期缺陷的版本还会失去本次生命周期保证，需单独评估。

本地 MySQL 迁移、约束与并发测试只证明本地版本行为；CI、发布版本及真实业务验收必须分别确认。设计与交付范围见[组合 Token 设计](../specs/2026-09-09-composite-token-design.md)。
