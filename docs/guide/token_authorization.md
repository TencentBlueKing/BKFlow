...
本文描述 2026-09-09 实现的组合票据与旧单项协议兼容行为。组合申请默认开启；发布时须先完成本次迁移和全部消费者适配，再启动新版本服务。部署版本不同时，应核对对应版本的权限类和配置；本地回归不表示 CI、生产部署或业务验收已完成。

面向 BKFara 等接入方的改造任务、请求示例、缓存与撤销策略及联调清单见 [组合 Token 接入改造指南](composite_token_integration.md)。
...
部署服务配置 `TOKEN_COMPOSITE_ENABLED` 默认开启，未设置环境变量时即可使用新申请格式；设置 `BKAPP_TOKEN_COMPOSITE_ENABLED=false` 可关闭。显式配置仍仅接受不区分大小写的字面量 `true` 启用，`1`、`0`、`false` 均不启用。开关只控制 grants 申请，关闭后已签发组合票据仍可消费、续期和撤销。
...
4. 迁移完成后读回旧票据，再启动全部新版本实例；grants 申请默认开启。确认旧协议、空间/用户隔离、过期拒绝及 SDK/页面调用正常。需要暂缓或停止新格式申请时，可显式设置 `BKAPP_TOKEN_COMPOSITE_ENABLED=false`。开关只控制组合申请，不控制已签发票据的读取、续期或撤销。
