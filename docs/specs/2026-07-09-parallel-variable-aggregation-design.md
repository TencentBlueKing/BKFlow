...
> 交互细节见第 6 节；高保真设计稿见 `prototypes/output/parallel-variable-aggregation/designs/`（源文件 `hifi/`）。
...
**关键：现状不存在「覆盖」这个独立语义。** 两个节点输出到同一 key，走的是「变量复用」——它们引用**同一个全局变量对象**，但 web 到引擎的转换只接入第一个来源，运行期仍是单值，拿不到其他分支的结果。另有一条硬校验（key validator 最后一条）：`if (value in constants) return false`，提示「变量KEY值已存在」，阻止手动填同名 key 提交。

> 注：现网把**输出参数**勾选为全局变量时不经过 `ReuseVarDialog`（它只用于输入参数），KEY 冲突时直接生成带随机后缀的新变量。聚合向导需新建组件，交互与样式参考 `ReuseVarDialog`。
...
| **复用 result**（仍为单值） | 复用现有 | 变量复用 | 两节点引用同一全局变量，运行期仍是单值（只接入第一个来源），不是新开关 |
...
2. **分支 2** 也设 `result` → 在**节点配置仍打开**时弹出 `ReuseVarDialog`（可扩展为多步向导），`sameKeyExist` 时**新增「聚合」选项**：
   - 聚合到 result（升级为聚合变量）
   - 复用 result（沿用同名全局变量，仍为单值）
   - 手动创建（改名为独立变量）
3. 用户选「聚合」后，**仍在同一 Dialog / 节点配置语境内**进入下一步：配置**聚合方式**（列表默认 / 字典）+ 只读展示来源（分支标识对应的分支与节点）
...
高保真设计稿目录：`prototypes/output/parallel-variable-aggregation/`（`hifi/` 源文件、`designs/` 截图，逐项说明见该目录 README）。2026-09-24 由低保真线框与简易稿升级而来。

- [x] 01 同名输出：选择聚合（向导第 1 步）
- [x] 02 不聚合：手动创建或复用（第 2、3 步置灰）
- [x] 03 设置聚合方式（向导第 2 步，同一弹窗）
- [x] 04 确认下游引用（向导第 3 步）
- [x] 05 同一分支重复输出（阻断）
- [x] 06 新增分支自动并入
- [x] 07 删除聚合来源（降级确认）
- [x] 08 全局变量列表中的聚合变量（来源浮层、建议降级）
- [x] 09 运行结果：查看聚合值（任务全局变量侧滑）
...
- **步骤 2 · 聚合方式**（仅选聚合后可进）：默认列表；可选字典；只读展示来源表（分支标识 → 分支与节点）
- **步骤 3 · 引用确认**（有下游引用时）：逐条列出 `${result}` 引用点，与单值比较的分支条件标记「需改写」；勾选确认后才完成升级，系统不自动改写引用。不提供「先保存为草稿」（未开启版本管理的空间没有草稿）
...
**C. dict 模式分支标识展示**
- key 取分支序号（如 `branch_1`/`branch_2`），按并行网关出线顺序由系统生成，用户只读
- 值结构：列表 `[{"branch": "branch_1", "value": ...}]`，字典 `{"branch_1": ...}`；来源节点不写进值，只在向导、变量列表和运行结果中展示

**D. 增删分支联动**
- 新增分支写同名 key：自动并入，提示「已自动并入聚合变量」
- 删除来源：删除前弹确认，列出受影响变量及删除前后来源数；仅剩 1 个来源时默认勾选「降级为普通变量」，可取消，取消后变量列表显示「建议降级」

**E. 同分支重复**
- 同一分支内先后两个节点输出同名 key：阻断勾选并提示，不进入向导
...
- [ ] 同分支重复被阻断后用户没有改名入口：是否改为沿用现网行为，生成带随机后缀的独立变量并提示
- [ ] 保存校验「聚合变量至少 2 个来源」与「只剩 1 个来源时保持聚合」冲突：建议放宽为至少 1 个
- [ ] 删除分支后分支序号重新编号，字典模式下按键名取值的引用会失效：是否改为稳定标识
