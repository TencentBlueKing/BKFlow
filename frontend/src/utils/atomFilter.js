...
const atomFilter = {
  ...
  getVariableArgs(variable) {
    const { source_tag: sourceTag, custom_type: customType } = variable;
    let name = '';
    let atom = '';
    let tagCode = '';
    let classify = '';
    if (customType) {
      name = customType;
      atom = sourceTag ? sourceTag.split('.')[0] : customType; // 兼容旧数据自定义变量source_tag为空
      tagCode = sourceTag ? sourceTag.split('.')[1] : customType;
      classify = 'variable';
    } else {
      [name, tagCode] = sourceTag.split('.');
      atom = name;
      classify = 'component';
    }
