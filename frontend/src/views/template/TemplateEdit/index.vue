...
<template>
  <div
    v-bkloading="{
      isLoading: templateDataLoading || singleAtomListLoading,
      zIndex: 100,
    }"
...
export default {
    ...
    computed: {
        ...
        canvasData() {
            const locations = this.locations.map((location) => {
                this.validateConnectFailList = [
                    ...new Set(this.validateConnectFailList),
                ];
                const status = this.validateConnectFailList.includes(location.id)
                    ? 'FAILED'
                    : '';
        ...
        isViewMode() {
            return (
                this.type === 'view'
                || !this.tplActions.some(action => ['EDIT', 'MOCK'].includes(action))
            );
        },
        templateType() {
    ...
    methods: {
        ...
        ...mapActions('task', ['loadSubflowConfig']),
        ...mapActions('atomForm/', [
            'loadSingleAtomList',
            'loadAtomConfig',
            'loadPluginServiceMeta',
        ]),
        ...mapActions('project/', ['getProjectLabelsWithDefault']),
        ...
        ...mapMutations('atomForm/', ['clearAtomForm']),
        ...mapMutations(['setSpaceId']),
        ...mapGetters('template/', ['getLocalTemplateData', 'getPipelineTree']),
        ...mapActions('task/', ['loadTaskScheme', 'saveTaskSchemList']),
        // 轮询更新token
        pollingToken() {
        ...
        async getTemplateData() {
            try {
                ...
                if (this.type === 'clone') {
                    templateData.name = `${templateData.name.slice(
                        0,
                        STRING_LENGTH.TEMPLATE_NAME_MAX_LENGTH - 6
                    )}_clone`;
                }
         ...
         * 加载子流程输入参数表单及配置项
         */
        async getSubflowConfig(location) {
            // get subflow constants and add node
            try {
                const params = {
        ...
        async getNodeVariableCitedData() {
            try {
                ...
                const nodeCitedInfo = Object.keys(variableCited).reduce(
                    (acc, key) => {
        ...
        validateAtomNode() {
            ...
            if (!isAllValid) {
                let message =                    this.isParallelGwErrorMsg
                    || i18n.t('任务节点参数错误，请点击错误节点查看详情');
                if (this.typeOfNodeNameEmpty) {
                    message =                        this.typeOfNodeNameEmpty === 'serviceActivity'
                            ? i18n.t('请选择节点的插件类型')
                            : i18n.t('请选择节点的子流程');
                }
                if (errorId.length) {
        ...
        checkAtomData(config, formData) {
            ...
            config.forEach((item) => {
                ...
                if (type === 'combine') {
                    if (
                        typeof value === 'object'
                        && !this.checkAtomData(attrs.children, value)
                    ) {
                        // 勾选为全局变量的 combine 不校验 value
                        isValid = false;
                    }
                } else {
                    if (attrs.validation && attrs.validation.length) {
                        attrs.validation.forEach((item) => {
                            ...
                            if (item.type === 'custom') {
                                if (!/^\${[^${}]+}$/.test(value)) {
                                    // '${xxx}'格式的值不校验
                                    const validateInfo = item.args.call(
                                        formObj,
                                        value,
                                        formData
                                    );
            ...
            this.locations.forEach((location) => {
                if (location.type === 'tasknode') {
                    ...
                    if (
                        ...
                        const atom = this.atomList.find(item => nodeConfig
                                && nodeConfig.component.code === item.code);
                        if (atom) {
        ...
        async onShowNodeConfig(id) {
            ...
            if (
                nodeConfig
                && nodeConfig.type === 'ServiceActivity'
                && nodeConfig.name
                && isDefaultPlugin
            ) {
                let atom = true;
            ...
            if (['tasknode', 'subflow'].includes(location.type)) {
                ...
                if (
                    ...
                    const resp = await this.loadPluginServiceMeta({
                        plugin_code:
                            nodeConfig.component.data.plugin_code.value,
                    });
                    // 第三方插件是否存在
        ...
        async onLocationChange(type, node) {
            ...
            } else if (type === 'add' && location.type === 'convergegateway') {
                // 新增汇聚网关时，判断下是否有并行网关校验失败
                this.$nextTick(() => {
        ...
        checkParallelGwConnect() {
            const index = this.validateConnectFailList.findIndex((val) => {
                ...
                if (
                    gateway
                    && ['ParallelGateway', 'ConditionalParallelGateway'].includes(gateway.type)
                ) {
                    ...
                    result =                        branchSinkNodes.length === 1
        ...
        onLineChange(type, edge) {
            ...
            const line = {
                ...
                source: {
                    ...
                    arrow: `${sourceArrow
                        .charAt(0)
                        .toUpperCase()}${sourceArrow.slice(1)}`,
                },
                target: {
                    id: target.cell,
                    arrow: `${targetArrow
                        .charAt(0)
                        .toUpperCase()}${targetArrow.slice(1)}`,
                },
            };
            ...
            nodeList.forEach((node) => {
                let nodeInfo =                    this.activities[node]
                    || this.locations[node]
                    || this.gateways[node];
                if (!nodeInfo) {
                    nodeInfo = node === this.end_event.id ? this.end_event : {};
                    nodeInfo =                        node === this.start_event.id
                            ? this.start_event
                            : nodeInfo;
                }
                if (nodeInfo.id) {
        ...
        goBackToTplEdit() {
            ...
            const isEqual = isDefaultSchemeIng
                ? judgeDataEqual()
                : !this.isSchemaListChange;
            if (isEqual) {
        ...
        async checkNodeAndSaveTemplate() {
            ...
            if (!validateMessage.result) {
                ...
                const nodeObject = Object.assign(
                    {},
                    this.activities,
                    this.gateways
                );
        ...
        onCitedNodeClick(data) {
            ...
            if (group === 'activities') {
                ...
                const nodeId = this.lines.find(line => line.id === id).source
                    .id;
        ...
        onUseSnapshoot(args) {
            ...
            const name =                i18n.t('最近快照已保存，并恢复至原序号为')
                + index
                + i18n.t('的快照');
        ...
        getTplSnapshoots() {
            ...
            const tpl = this.type === 'edit' ? this.templateId : this.tplUUID;
            return tplSnapshoot
                .getTplSnapshoots(this.username, id, tpl)
                .slice()
                .reverse();
        },
        // 清除临时快照
        ...
        saveTempSnapshoot(templateId) {
            const id = this.spaceId;
            tplSnapshoot.replaceSnapshootTplKey(
                this.username,
                id,
                this.tplUUID,
                templateId
        ...
        async getSpaceRelatedConfig() {
            try {
                const resp = await this.loadSpaceRelatedConfig({
                    id: this.templateId,
                });
                if (resp.result) {
