<template>
  <div :class="['http-callback-wrapper', { 'is-task-setting': !isViewMode }]">
    <div class="http-callback-switch">
      <span>{{ $t('是否开启') }}</span>
      <bk-switcher
        v-model="isEnable"
        :disabled="isViewMode"
      ...
      <bk-tab
        ...
        <bk-tab-panel
          ...
          <div
            ...
            class="auth-bearer-info">
            <bk-form
              ref="tokenForm"
              form-type="vertical"
              :rules="tokenFormRules"
              :model="localWebhookForm.extra_info.authorization">
              <bk-form-item
                label="Token"
                property="token"
                :icon-offset="15"
            ...
            <bk-form
              ...
              <bk-form-item
                ...
                <div class="from-item-content">
                  <bk-input
                    v-model="localWebhookForm.extra_info.authorization.username"
                    behavior="simplicity"
                    :disabled="isDisabled"
                    :clearable="true"
                    @change="onWebhookConfigChange" />
              ...
              <bk-form-item
                ...
                <div class="from-item-content">
                  <bk-input
                    v-model="localWebhookForm.extra_info.authorization.password"
                    behavior="simplicity"
                    type="password"
                    :disabled="isDisabled"
                    :clearable="true"
          ...
          <bk-table
            ...
            <bk-table-column
              ...
              <template slot-scope="{ row }">
                <bk-popover
                  ...
                  <bk-input
                    v-model="row[headerFieldConfig[item.id]]"
                    behavior="simplicity"
                    :disabled="isDisabled"
                    :clearable="true"
                    @change="onWebhookConfigChange" />
            ...
            <template slot="append">
              <div class="header-table-bottom">
                <div
                  :class="['add-header', { 'add-header-disabled': isDisabled }]"
                  @click="isDisabled ? null : addHeadersRow()">
                  <bk-icon type="plus" />
                </div>
              </div>
            </template>
        ...
        <bk-tab-panel
          ...
          <bk-form
            ...
            :model="localWebhookForm.extra_info">
            <bk-form-item
              v-for="item in settingFieldConfig"
              :key="item.key"
              :label="item.label"
              :property="item.key"
              :icon-offset="27">
              <div class="from-item-content">
                <bk-input
                  v-model="localWebhookForm.extra_info[item.key]"
                  :disabled="isDisabled"
                  @change="onWebhookConfigChange" />
                <span
                  v-if="item.key !== 'retry_times'"
                  class="unit">s</span>
                <span
                  v-else
                  class="unit">{{ $t('次') }}</span>
...
<script>
    ...
    export default {
        ...
        data() {
            ...
            return {
                ...
                settingFormRules: {
                    retry_times: [
                        {
                            validator: val => this.checkData(val, true),
                ...
                addrFormRules: {
                    endpoint: [
                        {
                            validator: val => this.checkData(val),
        ...
        computed: {
            isDisabled() {
                return this.isViewMode || !this.isEnable;
            },
            isAccessAuth() {
            ...
            isAccessSettings() {
                const { timeout, retry_times: retryTimes, interval } = this.localWebhookForm.extra_info;
                const isNumber = /^[0-9]+$/.test(timeout) && /^[0-9]+$/.test(retryTimes) && /^[0-9]+$/.test(interval);
                return isNumber && timeout <= 10 && retryTimes <= 5 && interval <= 600;
            },
        },
        methods: {
            ...mapActions('template', [
