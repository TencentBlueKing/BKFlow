  ...
  <div class="template-list">
    ...
    <bk-table
        ...
        <template slot-scope="props">
          ...
          <div
            ...
            <bk-popover
              ...
              <bk-button
                v-if="
                  getSubflowUpdateCount(
                    props.row.subprocess_info
                  ) > 0
                "
                :text="true">
                <span class="red-text">
      ...
      <div
        ...
        {{ $t("当前已选择 x 条数据", { num: selectedTpls.length })
        }}{{ $t("，") }}
        <bk-link
          theme="primary"
          @click="selectedTpls = []">
      ...
      <div class="copy-del-dialog-content">
        ...
        <div class="mock-text">
          <div>
            {{
              $t(
                "关联的mock数据不会同步复制，暂不支持复制带有决策表节点的流程"
              )
            }}
          </div>
        </div>
        <bk-checkbox v-model="isSelectCopySubflow">
          {{ $t("复制包含的子流程") }}
        </bk-checkbox>
      </div>
    </bk-dialog>
...
export default {
    ...
    methods: {
            ...
            return h(
                'div',
                {
                    class: {
                        ...
                        'full-selected':
                            this.pagination.count === this.selectedTpls.length,
                    },
                },
                [
                    h('bk-checkbox', {
        ...
        onBatchDelete() {
            ...
            this.$bkInfo({
                title: this.$t('确认删除所选的 {0} 项流程吗?', [
                    this.selectedTpls.length,
                ]),
                maskClose: false,
                width: 450,
                confirmLoading: true,
