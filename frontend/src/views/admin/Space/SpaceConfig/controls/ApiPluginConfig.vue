    ...
    <bk-form
      ...
      <div
        ...
        <div class="apc-test-wrap">
          <bk-button
            theme="primary"
            :disabled="apiItem.testLoading || !(apiItem.apiKey && apiItem.display_name && apiItem.meta_apis)"
          ...
          <template v-if="apiItem?.testResult">
            ...
            <span
              v-if="apiItem.testResult?.ok">
              <span class="test-text">{{ $t('测试成功：接口 {n} 个，分类 {m} 个', { n: apiItem.testResult.data.total || 0, m: apiItem.testResult.data.sample.length || 0}) }}</span>
              <bk-popover
                v-if="apiItem.testResult.data.total > 0"
                theme="light"
                placement="bottom"
                width="520"
                ...
                <div slot="content">
                  <bk-table
                    :data="getExampleList(apiItem.testResult)"
                    :max-height="300"
                    size="small">
                    <bk-table-column
                      :label="$t('接口')"
  ...
  export default {
      ...
      return {
        ...
        rules: {
          ...
          meta_apis: [
            {
              required: true,
              message: this.$t('请输入 Meta 接口 URL'),
              trigger: 'submit',
            },
          ],
        },
    ...
    methods: {
      ...
      getExampleList() {
        ...
        const examples = [
                { name: 'getBkChatMessages', method: 'GET', path: '/api/v1/bk_chat/dify/api/get_messages/' },
                { name: 'sendBkChatMessage', method: 'POST', path: '/api/v1/bk_chat/dify/api/send_message/' },
                { name: 'listTasks', method: 'GET', path: '/api/v1/task/list/' },
                { name: 'createTask', method: 'POST', path: '/api/v1/task/create/' },
                { name: 'getTaskDetail', method: 'GET', path: '/api/v1/task/{id}/' },
                { name: 'updateTask', method: 'PUT', path: '/api/v1/task/{id}/update/' },
                { name: 'deleteTask', method: 'DELETE', path: '/api/v1/task/{id}/' },
                { name: 'listTemplates', method: 'GET', path: '/api/v1/template/list/' },
                { name: 'getTemplateDetail', method: 'GET', path: '/api/v1/template/{id}/' },
                { name: 'executeFlow', method: 'POST', path: '/api/v1/flow/execute/' },
              ];
        return examples;
      },
    },
