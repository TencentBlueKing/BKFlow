  ...
  export default {
    ...
    methods: {
      ...
      getV4Component(variable, atom, version) {
        ...
        const data = variable.component_data || variable.data || variable;
        return {
      ...
      async loadV4VariableForm(variable, atom, version, generation, runtimeInputs) {
      ...
      appendJsonSchemaField(schema, formSchema, variable, tagCode) {
        ...
        let sourceKey = '';
        if (properties[tagCode]) {
          sourceKey = tagCode;
        } else if (properties[variable.key]) {
          sourceKey = variable.key;
        }
        const mergedSchema = mergeV4ObjectSchema(schema, formSchema, {
      ...
      async getFormData() {
        ...
        for (const variable of variableArray) {
          ...
          const v4Result = await this.loadV4VariableForm(variable, atom, version, generation, nextRenderData);
          if (!isCurrentGeneration()) return;
