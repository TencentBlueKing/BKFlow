...
<template>
  <div
    ...
    <div class="node-status-block">
      <i class="node-icon-font common-icon-subflow-mark" />
      <div
    ...
    <!-- 节点右上角执行相关的icon区域 -->
    <node-right-icon-status :node="node" />
    ...
    <div class="state-icon">
      <template v-if="node.task_state !== 'REVOKED'">
        ...
        <span
          v-if="!isSubProcessNode && node.status === 'SUSPENDED'"
      ...
      <span
        v-if="!isSubProcessNode"
        @click.stop="$emit('onSubflowDetailClick', node.id)">
        <i class="common-icon-bkflow-setting" />
        {{ $t('节点参数') }}
      </span>
    </div>
  </div>
