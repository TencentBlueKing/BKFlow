<template>
  <div
    ref="subflowListPanel"
    class="subflow-list-panel">
    <div class="search-wrap">
      ...
      <template v-if="!common && isFilterLabelMode">
        ...
        <div
          class="thumb-icon"
          @click="handleChangeSearchMode('text')">
      ...
      <template v-else>
        <div
          v-if="!common"
          class="thumb-icon"
          @click="handleChangeSearchMode('label')">
  ...
  export default {
    ...
    methods: {
      ...
      async getTplList() {
        ...
        try {
          ...
          resp.results.forEach((tpl) => {
            const tplCopy = {
              ...tpl,
              hasPermission: this.hasPermission(reqPermission, tpl.auth_actions),
              tplSource: this.common ? 'common' : 'business',
            };
            // 高亮搜索匹配的文字部分
            if (searchStr !== '') {
      ...
      onApplyPermission(node) {
        let reqPerm; let permissionData;
        if (this.common) {
