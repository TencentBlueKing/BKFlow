...
<template>
  <div class="loop-rule-select">
          ...
          <div class="tabpanel-container">
            <bk-radio-group
              ...
              <span
                ...
                @click.stop="onResetGroupValue(item)">{{ $t('清空') }}</span>
            </bk-radio-group>
            <!-- 循环生成 -->
            <div
              v-if="item.radio === 0"
              ...
              <i
                v-if="item.key === 'week'"
                v-bk-tooltips="$t('0 表示星期天，6 表示星期六')"
                class="common-icon-info month-tips top-start" />
              <!-- startInput 错误提示 -->
              <div
                v-show="veeErrors.has(item.key + 'Rule') || veeErrors.has('interval')"
    ...
    <i
      v-bk-tooltips="ruleTipsHtmlConfig"
      class="common-icon-info rule-tips" />
    <!-- corn 规则 tips -->
    ...
    <!-- 手动输入错误提示 -->
    <span
      v-show="veeErrors.has('periodicCron') && currentWay === 'manualInput'"
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      // 清空已选
      clearRule() {
        this.autoRuleList.forEach((item) => {
          item.checkboxList.forEach((m) => {
            m.checked = false;
          });
      ...
      renderRule() {
        this.autoRuleList.forEach((m, i) => {
          ...
          const pointRule = checkboxList
            ...
            .map((res) => {
              const result = loopStarOneList.includes(m.key) ? res.v + 1 : res.v;
              return result;
            })// start 1 时 显示 i + 1
            .join(',') || '*';
      ...
      mergeCloseNumber(arr) {
        if (Array.isArray(arr)) {
          ...
          return exportList.map((m) => {
            const result = m.length > 1 ? `${m[0]}-${m[m.length - 1]}/1` : `${m[0]}`;
            return result;
          });
        }
        return arr;
        ...
        this.$nextTick(() => {
          ...
          periodicList.forEach((m, i) => {
            ...
            if (m === '*') {
              ...
              // d,d,d,d
              item.radio = 1;
              item.checkboxList.forEach((box) => {
                box.checked = m.split(',').some((s) => {
                  const result = loopStarOneList.includes(item.key) ? s * 1 - 1 === box.v * 1 : s * 1 === box.v * 1;
                  return result;
                });
              });
            } else {
