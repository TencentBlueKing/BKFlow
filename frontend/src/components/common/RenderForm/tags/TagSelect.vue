        ...
        componentUpdated (el, binding, vnode) {
          ...
          if (multiple) {
            ...
            seletedValue.forEach((val, index) => {
              ...
              if (hasGroup) {
                valInOptions = options.some(group => group.options.some(item => {
                  if (item.value === val) {
                    return true
                  }
                }))
      ...
      viewValue () {
        if (Array.isArray(this.seletedValue)) { // 多选
          ...
          if (this.items.length) {
            return this.seletedValue.map(val => this.filterLabel(val)).join(',')
          }
    ...
    methods: {
      ...
      handleSelectPaste (event) {
        ...
        paste.forEach(item => {
          ...
          const option = this.items.find(option => iaMatch ? option.text === item : item.toLowerCase() === option.text.toLowerCase())
          if (option) {
