const en = {
  ...
  蓝鲸应用凭证: 'BK App Credential',
  '蓝鲸 Access Token 凭证': 'BK Access Token',
  ...
  取消发布: 'Cancel Publish',
  '版本号/版本描述/操作': 'Version Number/Version Description/Operation',
  最新版本不能删除: 'Latest version cannot be deleted',
  发布流程: 'Publish Flow',
  请输入版本号: 'Please enter version number',
  发布成功: 'Publish Success',
  '确认删除当前版本"{0}"?': 'Confirm delete current version "{0}"?',
  草稿版本: 'Draft Version',
  ...
  查看内容: 'View content',
  作用域: 'Scope',
  凭证指引: 'Credential guidelines',
  凭证内容: 'Credential content',
  作用域值: 'Scope value',
  作用域类型: 'Scope type',
  开放范围: 'Open scope',
  不开放: 'Not open',
  全部流程: 'All processes',
  按作用域开放: 'Open by scope',
  查看凭证获取指引: 'View credential acquisition guide',
  蓝鲸应用认证: 'BlueKing App Authentication',
  至少保留一个: 'Keep at least one',
  名称不能为空: 'The name cannot be empty',
  bk_app_code不能为空: 'bk_app_code cannot be empty',
  bk_app_secret不能为空: 'bk_app_secret cannot be empty',
  access_token不能为空: 'access_token cannot be empty',
  username不能为空: 'username cannot be empty',
  password不能为空: 'password cannot be empty',
  凭证内容或者作用域存在重复的key: 'There are duplicate keys: {value}',
  第n项凭证内容不能为空: 'The content of the {value} credential cannot be empty',
  第n项凭证作用域不能为空: 'The scope of the {value} credential cannot be empty',
  '第n项凭证内容格式错误, 仅支持字母、数字、下划线、连字符': 'The format of the {value} credential content is incorrect, only supporting letters, numbers, underscores, and hyphens',
  ...
  固定循环次数: 'Fixed Loop Count',
  最大并行数: 'Maximum Parallelism',
  循环类型: 'Loop Type',
  循环变量: 'Loop Variable',
  请输入循环变量: 'Please Enter Loop Variable',
  循环次数: 'Loop Count',
  输入变量: 'Input Variable',
  ...
  流程与画布行为: 'Flow and Canvas Behavior',
  'API 与插件集成': 'API and Plugin Integration',
  其他: 'Other',
  已配置: 'Configured',
  异常: 'Error',
  查看文档: 'View Documentation',
  示意图待补充: 'Diagram to Be Added',
  测试: 'Test',
  验证通过: 'Verification Passed',
  已恢复默认值: 'Default Restored',
  '确认恢复默认值？': 'Confirm to reset to default value?',
  '切换 JSON 源码': 'Switch to JSON Source',
  切换结构化编辑: 'Switch to Structured Editor',
  默认凭证: 'Default Credential',
  '请选择 BK_APP 凭证': 'Please select BK_APP credential',
  '凭证不存在，请重选': 'Credential does not exist, please reselect',
  按作用域覆盖: 'Override by Scope',
  添加覆盖: 'Add Override',
  '当前值不是标准 API 结构，请使用右上角"切换 JSON 源码"编辑': 'The current value is not a standard API structure. Please use "Switch to JSON Source" at the top right to edit',
  'api_key，如 default': 'api_key, e.g. default',
  显示名称: 'Display Name',
  '接口列表 apigw URL': 'API list apigw URL',
  '分类接口 apigw URL，可选': 'Category API apigw URL, optional',
  添加请求头: 'Add Header',
  '测试通过，共 {0} 个 API': 'Passed, {0} APIs in total',
  测试通过: 'Passed',
  '添加 API': 'Add API',
  允许名单: 'Allow List',
  仅所列插件可用: 'Only listed plugins are available',
  屏蔽名单: 'Deny List',
  '屏蔽所列插件，其余可用': 'Block listed plugins, others available',
  '选择或输入插件 code': 'Select or input plugin code',
  空间级: 'Space Level',
  按作用域: 'By Scope',
  键: 'Key',
  值: 'Value',
  添加: 'Add',
  添加作用域: 'Add Scope',
  添加作用域覆盖: 'Add Scope Override',
  作用域重复: 'Duplicate scope',
  请选择作用域: 'Please select scope',
  '请输入值, 如: { "key1": "value1" }': 'Please enter value, e.g. { "key1": "value1" }',
  '请输入作用域,如: {scope_type}_{scope_value}': 'Please enter scope, e.g. {scope_type}_{scope_value}',
  '作用域格式为 {scope_type}_{scope_value}，每个作用域对应一个值（字符串/数字/布尔）': 'Scope format is {scope_type}_{scope_value}; each scope maps to one value (string/number/boolean)',
  '键值仅支持字符串/数字/布尔': 'Keys and values only support string/number/boolean',
  '请输入值（字符串/数字/布尔）': 'Please enter a value (string/number/boolean)',
  '作用域格式应为 scope_type_scope_value': 'Scope format should be scope_type_scope_value',
  请选择默认凭证: 'Please select default credential',
  测试有效性: 'Test Validity',
  恢复默认: 'Restore Default',
  测试链接: 'Test Link',
  重新测试: 'Retest',
  '测试中...': 'Testing...',
  '测试成功：接口 {n} 个，分类 {m} 个': 'Test successful: {n} APIs, {m} categories',
  '测试失败：': 'Test failed: ',
  测试失败: 'Test Failed',
  查看样例: 'View Examples',
  未知错误: 'Unknown Error',
  '调用网关时按此表取凭证鉴权，表中作用域未命中时回落到默认凭证。': 'When calling the gateway, use this table to obtain credentials for authentication; if the scope in the table does not match, fall back to the default credential.',
  效果演示: 'Effect Demo',
  前往凭证管理: 'Go to Credential Management',
  '新增 api_key': 'Add api_key',
  '改动可能影响流程编排中「API插件」可选的接口与分类中已使用该接入的流程，请谨慎修改！': 'This change may affect the available interfaces and categories for "API Plugins" within process orchestration, as well as any processes already utilizing this integration; please exercise caution when making modifications.',
  展示名称: 'Display Name',
  'Meta 接口 URL': 'Meta Api URL',
  '分类接口 URL': 'Category Interface URL',
  请求头: 'Request headers',
  接口: 'Interface',
  未分类: 'Unclassified',
  请输入作用域: 'Please enter scope',
  '按作用域覆盖(可根据自身需要判断是否需要配置)': 'Override by scope (optional, configure as needed)',
  '请输入作用域,如：{scope_type}_{scope_id}': 'Please enter scope, e.g. {scope_type}_{scope_id}',
  '请输入 api_key': 'Please enter api_key',
  请输入展示名称: 'Please enter display name',
  请输入Meta接口URL: 'Please enter Meta Api URL',
  '请输入api_key，如 default': 'Please enter api_key, e.g. default',
  '如 apigw.example.com/api/{api_key}/meta': 'e.g. apigw.example.com/api/{api_key}/meta',
  '如 apigw.example.com/api/{api_key}/categories': 'e.g. apigw.example.com/api/{api_key}/categories',
  请输入key: 'Please enter key',
  请输入value: 'Please enter value',
  测试配置: 'Test Configuration',
  '允许名单:仅所列插件可用': 'Allow List: Only listed plugins are available',
  ...
  用途: 'Purpose',
  影响: 'Impact',
  '控制流程画布插件面板里标准插件的可见范围。按「仅显示」或「隐藏」过滤，保存后立即生效。': 'Controls the visibility of standard plugins in the process canvas plugin panel. Filters by "Show Only" or "Hide" and takes effect immediately after saving.',
  '被过滤的标准插件从画布面板消失，不能再拖入新节点。': 'Filtered standard plugins disappear from the canvas panel and can no longer be dragged in as new nodes.',
  '已有流程若用了这些插件，打开节点配置时可能提示「未找到」，无法继续编辑。': 'If existing processes use these plugins, opening their node configuration may show "Not Found" and prevent further editing.',
  '「仅显示」按插件 code 白名单生效。名单里没有 API 插件 / 第三方插件时，它们的入口也会从面板消失，存量 API 节点同样可能无法打开。': '"Show Only" works as a plugin code whitelist. If API plugins / third-party plugins are not in the list, their entries also disappear from the panel, and existing API nodes may fail to open.',
  '已在运行的任务不会因此停掉；但编辑受阻后无法改这些节点再保存。': 'Running tasks will not stop because of this; however, once editing is blocked, these nodes cannot be modified or saved again.',
  '此配置会影响已有流程编辑。保存前请确认名单覆盖本空间已在使用的插件（含 API 插件）。': 'This configuration affects editing of existing processes. Before saving, make sure the list covers all plugins already in use in this space (including API plugins).',
  '不过滤(默认)：画布展示空间内全部可用插件': 'No filtering (default): the canvas displays all plugins available in the space',
  '仅显示名单中的插件：面板只留所选插件；未列入的标准插件、以及未列入的 API / 第三方入口都会消失': 'Only show plugins in the list: the panel keeps only the selected plugins; unlisted standard plugins, as well as unlisted API / third-party entries, will all disappear',
  '隐藏名单中的插件：只从面板拿掉所选标准插件，其余（含 API 插件）仍可见。已有流程更安全': 'Hide plugins in the list: only removes the selected standard plugins from the panel; the rest (including API plugins) remain visible. Safer for existing processes',
  插件: 'Plugin',
  '确认保存空间插件配置？': 'Confirm saving the space plugin configuration?',
  '保存后立即生效，可能影响本空间已有流程：': 'Once saved, it takes effect immediately and may affect existing processes in this space:',
  '画布插件面板将按名单过滤，未覆盖的标准插件不再显示': 'The canvas plugin panel will be filtered by the list; uncovered standard plugins will no longer be displayed',
  '已使用这些插件的流程，打开节点时可能提示「未找到」，无法继续编辑': 'For processes already using these plugins, opening their nodes may show "Not Found" and prevent further editing',
  '使用「仅显示」且未把 API 插件列入时，第三方 / API 插件入口也会消失，存量 API 节点同样可能无法打开': 'When using "Show Only" without listing API plugins, third-party / API plugin entries will also disappear, and existing API nodes may fail to open',
  ...
  语法要点: 'Syntax Essentials',
  常用示例: 'Common Examples',
  场景: 'Scenario',
  完整语法请参考: 'For full syntax, please refer to',
  比较运算符: 'Comparison Operators',
  逻辑关键词: 'Logical Keywords',
  '支持 ${key}、${int(key)}': 'Supports ${key}, ${int(key)}',
  字符串比较: 'String Comparison',
  数值比较: 'Numeric Comparison',
  包含判断: 'Contains Check',
  '前提：status / retry / env 为流程全局变量。': 'Prerequisite: status / retry / env are flow global variables.',
  'Boolrule 表达式文档': 'Boolrule Expression Documentation',
  '支持 list contains': 'Supports list contains',
  '前提：字符串比较用 =, 不要写成 ==。': 'Prerequisite: use = for string comparison, not ==.',
  'bkflow-feel 语法文档': 'bkflow-feel Syntax Documentation',
  '直接引用变量名 key, 不要包 ${}': 'Reference the variable name key directly, do not wrap it with ${}',
  '因 MAKO 语法限制：请勿在表达式中使用 $、{、} 字符。误用会导致分支条件解析失败。': 'Due to MAKO syntax limitations: do not use the $, {, } characters in expressions. Misuse may cause branch condition parsing failure.',
